...
const Home = () => {
  ...
  useEffect(() => {
    axios.post(`${process.env.NEXT_PUBLIC_API_URL}/search`, form)
    .then((res) => {
        setUser({
            ...user,
            isLoading: false,
  ...
  return (
    <div className={styles.container}>
                ...
                <div style={{height: '90%', width: '2px', backgroundColor: '#9EA0A5', margin: '0 1rem'}} />
                    <UncontrolledDropdown>
                        ...
                        <DropdownMenu aria-labelledby="dropdownMenuButton">
                            <DropdownItem value="name" onClick={(e) => setForm({...form, order: e.target.value})}>
                                Nama
                            </DropdownItem>

                            <DropdownItem value="skill" onClick={(e) => setForm({...form, order: e.target.value})}>
                                Skill
                            </DropdownItem>

                            <DropdownItem value="city" onClick={(e) => setForm({...form, order: e.target.value})}>
                                Lokasi
                            </DropdownItem>

                            <DropdownItem value="job_title" onClick={(e) => setForm({...form, order: e.target.value})}>
                                Title
                            </DropdownItem>
                        </DropdownMenu>
                    </UncontrolledDropdown>
        ...
        <div className={`${styles.paged} d-flex flex-row justify-content-center gap-3`}>
            <button className={`${styles.pageBox} d-flex align-items-center justify-content-center rounded-1 shadow-sm`} disabled={form.page <= 1} onClick={onPrev}><FontAwesomeIcon icon={faAngleLeft} /></button>
            ...
            <button className={`${styles.pageBox} d-flex align-items-center justify-content-center rounded-1 shadow-sm`} disabled={user.data.length === 0} onClick={onNext}><FontAwesomeIcon icon={faAngleRight} /></button>
        </div>
    </div>
  )
}
